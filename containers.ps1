...
docker build -t my_backend -f .\backend\dockerfile .

# Run Flask application container
docker run --name my_backend --network my_net -p 5000:5000 my_backend

# Build Frontend application Docker image
docker build -t my_frontend -f .\Frontend\your-project-name\dockerfile .
