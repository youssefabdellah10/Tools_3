...
export class ManageOrdersComponent implements OnInit {
  ...
  private async fetchCouriers(): Promise<void> {
    try {
      const response = await fetch('https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/couriers');
  ...
  private async fetchOrders(): Promise<void> {
    try {
      const response = await fetch('https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/admin/AllOrders');
  ...
  private async handleOrderUpdate(endpoint: string, payload: any): Promise<void> {
    try {
      const response = await fetch(`https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/${endpoint}`, {
        method: 'PUT',
        headers: {
  ...
  async deleteOrder(orderId: number): Promise<void> {
    if (confirm('Are you sure you want to delete this order?')) {
      try {
        const response = await fetch(`https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/admin/delete/order?order_id=${orderId}`, {
          method: 'DELETE',
        });
