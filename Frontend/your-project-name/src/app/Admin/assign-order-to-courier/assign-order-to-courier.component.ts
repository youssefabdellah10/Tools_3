...
export class AssignOrderToCourierComponent implements OnInit {
  ...
  private async fetchAssignedOrders() {
    try {
      const response = await fetch('https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/admin/assigned-orders');
      if (!response.ok) {
  ...
  async reassignOrder(orderId: number, newCourierId: number) {
    ...
    try {
      const response = await fetch('https://tools-3-backend-youssefabdellah10-dev.apps.rm3.7wse.p1.openshiftapps.com/AssignOrder', {
        method: 'PUT',
        headers: {
